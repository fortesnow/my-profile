...
export const AnimatedTestimonials = ({
  ...
  // 各testimonialに対して固定の回転値を生成（メモ化）
  const rotationValues = useMemo(() => {
    if (!isClient) {
      // サーバーサイドでは固定値を使用
      return testimonials.map((_, index) => (index % 3) * 3 - 3);
    }
    // クライアントサイドでランダム値を生成
  ...
  return (
    <div className={cn("max-w-sm md:max-w-4xl mx-auto px-4 md:px-8 lg:px-12 py-20 overflow-x-hidden", className)}>
